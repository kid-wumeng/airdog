...
test('Remove Cookie', function(done){
  client.get('/remove', function(res, body){
    ...
    res.headers['set-cookie'][2].should.equal('age=; Max-Age=-100000; Path=/user;')
    done()
  })
})
...
test('Remove All Cookies', function(done){
  ...
  client.get('/remove-all-op', options, function(res, body){
    res.headers['set-cookie'][0].should.equal('name=; Max-Age=-100000; Path=/user;')
    res.headers['set-cookie'][1].should.equal('age=; Max-Age=-100000; Path=/user;')
    done()
  })
})