...
Server.prototype.all = function(path, mid, config){
  return this.add(RouteTable.ALL, path, mid, config)
}
...
Server.prototype.post = function(path, mid, config){
  return this.add(RouteTable.POST, path, mid, config)
}



Server.prototype.put = function(path, mid, config){
  return this.add(RouteTable.PUT, path, mid, config)
}



Server.prototype.del = function(path, mid, config){
  return this.add(RouteTable.DELETE, path, mid, config)
}



Server.prototype.options = function(path, mid, config){
  return this.add(RouteTable.OPTIONS, path, mid, config)
}
...
Server.prototype.getCompletePath = function(path){
  switch(path){
    case '*':
    case '/': return path
    default:  return Kit.formatPath(`/${this.base}/${path}/`)
  }
}
