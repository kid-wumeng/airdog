...
class Application {
  ...
  call( req, res, reqData ){
    ...
    let isStatic = this.isStatic( path )
    let mids = []
    if(isStatic){
      // Static route
      console.log('static');
      mids.push({fn: this.static})
    } else {
      // Dynamic route
      console.log('dynamic');
  ...
  isStatic( path ){
    ...
    let STATICS = this.config.static || []
    if(typeof STATICS === 'string'){
      STATICS = [ STATICS ]
    }
    
    for( let i = 0; i < STATICS.length; i++ ){
      let staticPath = Path.normalize(`/${STATICS[i]}/`)
  ...
  async static(){
    let path = Path.normalize(process.cwd() + this.path)
