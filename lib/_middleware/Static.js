...
exports.appCreate = async function(){
  this.app.static = this.app.static || {}
  let dir = this.app.static['dir']

  if( typeof dir === 'string' )
    this.app.static['dir'] = [dir]
}
...
exports.mid = async function(){
  ...
  // Is use the static service
  if( !this.app.static['dir'] ){ return }
...
async function cache(local){
  ...
  if(shouldFlush(time)){
    this.set('Last-Modified', time.server.toUTCString())
    return true
  } else {
    this.status = 304
    this.body = ''
...
function shouldFlush(time){
  let client = time.client
  let server = time.server
  // Don't use the date.getTime() or date.valueOf()
  // because of date.toUTCString() isn't include the milliSeconds
  return client === null || Date.parse(client) < Date.parse(server)
}
